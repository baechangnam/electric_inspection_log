...
class KoHighHeader {
  ...
  static void applyRows30to35(
    ...
    required double currentGenerationKwh,
    ...
    block(
      topRow: 32,
      title: '현재  발 전 량',
      value: fmt(currentGenerationKwh),
      unit: 'KWH',
    );
  ...
  static void applyRows30to35FromEntry(
    ...
    applyRows30to35(
      ...
      currentGenerationKwh: entry.currentGenerationKwh,
  ...
  static void applyGuidelineRows(
    xlsio.Worksheet s,
    SimpleHvLogEntry e, {
    int startRow = 43, // 예: 결과블록(40~42) 아래면 43부터
  }) {
    xlsio.Range rg(String a1) => s.getRangeByName(a1);
    ...
    // 보기좋게 행높이
    for (var r = startRow; r <= startRow + 1; r++) {
      if (s.getRangeByIndex(r, 1).rowHeight < 22) {
        s.getRangeByIndex(r, 1).rowHeight = 22;
      }
    }
    ...
    cell('I$r1:K$r1', text: fmt(e.guidelineCurrent5));
    cell('L$r1:L$r1', text: '④', bold: true);
    cell('M$r1:O$r1', text: fmt(e.guidelineCurrent4));
    cell('P$r1:R$r1', text: '금일지침계', bold: true);
